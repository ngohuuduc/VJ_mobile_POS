...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-I01 | Logo công ty lấy từ đâu? Upload trong app hay từ Odoo (`res.company`)? | Template design, data source | Chờ | bổ xung trong thư mục assets. |
| OQ-I02 | Thông tin công ty (tên, địa chỉ, MST, SĐT) lấy từ đâu? Config trong app hay đọc từ Odoo `res.company`? | Template data, config | Chờ | Hardcode torng file html template |
| OQ-I03 | Có cần in số tiền bằng chữ (tiếng Việt) trên phiếu không? | PDF content, backend logic | Chờ | có |
| OQ-I04 | PDF sau khi generate: mở tab mới để in hay download file? | UX, frontend | Chờ | để preview|
| OQ-I05 | Admin chỉnh sửa template theo dạng nào? Soạn thảo HTML/CSS trực tiếp hay có visual editor? | Admin UI, độ phức tạp | Chờ | có visual editor|
| OQ-I06 | Mỗi loại phiếu có 1 template cố định hay có thể tạo nhiều template rồi chọn? | Data model template | Chờ | mỗi loại chỉ có 1 template cố định. |
| OQ-I07 | Phiếu có cần chữ ký / vùng ký tên không? (ví dụ: "Người mua" / "Nhân viên bán hàng") | Template design | Chờ | có vùng ký để ký sau khi in|
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-J01 | Có cho phép tạo đơn không có thông tin khách hàng (walk-in customer) không? | Luồng tạo đơn, data model | Chờ | |
| OQ-J02 | Khi tạo khách hàng mới từ Mobile POS, những trường nào bắt buộc? (tên, SĐT, MST, địa chỉ?) | UI form khách hàng | Chờ | |
| OQ-J03 | Tìm kiếm khách hàng theo tiêu chí nào? (tên, SĐT, MST — 1 hay nhiều tiêu chí?) | UI tìm kiếm | Chờ | |
| OQ-J04 | POS User có thể chỉnh sửa thông tin khách hàng (res.partner) không, hay chỉ ADMIN? | Phân quyền, Odoo write | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-K01 | Sản phẩm có variant (màu sắc, kích cỡ...) không? Nếu có, hiển thị chọn variant thế nào? | UI tạo đơn, Odoo model | Chờ | |
| OQ-K02 | Tìm kiếm sản phẩm theo tiêu chí nào? (tên, mã SKU, barcode?) | UI tạo đơn | Chờ | |
| OQ-K03 | Khi sản phẩm hết hàng tại location, có cho phép thêm vào đơn không? (cảnh báo hay chặn?) | Logic validate | Chờ | |
| OQ-K04 | Sản phẩm có ảnh không? Cần hiển thị ảnh trong danh sách sản phẩm không? | UI, cache size | Chờ | |
| OQ-K05 | Có cần nhóm/danh mục sản phẩm để lọc khi tạo đơn không? | UI tạo đơn | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-L01 | Danh sách đơn hàng filter theo tiêu chí nào? (ngày, trạng thái, nhân viên, khách hàng?) | UI, API query | Chờ | |
| OQ-L02 | Lịch sử đơn hiển thị bao xa? (hôm nay, 30 ngày, tất cả?) | Performance, UX | Chờ | |
| OQ-L03 | POS User chỉ thấy đơn của mình hay tất cả đơn trong location? | Phân quyền, query | Chờ | |
| OQ-L04 | Khi thu phần còn lại (đặt cọc), tìm đơn theo tiêu chí gì? (tên KH, SĐT, mã đơn?) | Luồng đặt cọc | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-M01 | Access token TTL bao lâu? Refresh token TTL bao lâu? | Auth, UX | Chờ | |
| OQ-M02 | Khi user bị ADMIN vô hiệu hóa, access token hiện tại có bị invalidate ngay không? | Auth security | Chờ | |
| OQ-M03 | Sau bao lâu không hoạt động thì tự đăng xuất (idle timeout)? | UX, security | Chờ | |
| OQ-M04 | Có cần giới hạn số lần đăng nhập sai (brute force protection) không? | Security | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-N01 | Khi Odoo không phản hồi (timeout/down), hiển thị gì và user làm gì tiếp? | UX, error handling | Chờ | |
| OQ-N02 | Khi XML-RPC call thất bại giữa chừng (đã tạo sale.order nhưng picking chưa tạo), xử lý thế nào? | Data integrity | Chờ | |
| OQ-N03 | Khi tồn kho cache (5 phút) không khớp với Odoo thực tế lúc validate, flow xử lý thế nào? | Stock validation | Chờ | |
| OQ-N04 | Odoo XML-RPC timeout được set bao lâu? | Performance, infra | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-O01 | 1 đơn có thể đặt cọc nhiều lần không? (vd: cọc lần 1 rồi cọc thêm lần 2?) | Luồng cọc, data model | Chờ | |
| OQ-O02 | Đơn đặt cọc có thời hạn hết hiệu lực không? (vd: giữ cọc 30 ngày?) | Business rule | Chờ | |
...
| ID | Câu hỏi | Ảnh hưởng đến | Trạng thái | Câu trả lời |
|---|---|---|---|---|
| OQ-P01 | Visual editor là loại gì? WYSIWYG đơn giản (GrapesJS, TinyMCE) hay chỉ là code editor với preview? | Độ phức tạp, frontend | Chờ | |
| OQ-P02 | Admin có thể preview phiếu trước khi lưu template không? | UX admin | Chờ | |
| OQ-P03 | Có cần version history cho template (lưu bản trước để rollback) không? | Data model | Chờ | |
| OQ-P04 | Khi template bị lỗi HTML/CSS, WeasyPrint fail — cần hiển thị lỗi cụ thể cho Admin không? | Error handling | Chờ | |
